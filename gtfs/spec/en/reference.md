...
| `bikes_allowed` | Enum | Optional | Overrides `trips.bikes_allowed` to provide stop-specific boarding information on the trip.<br><br>`0` or empty - No specific bike information for boarding/alighting at this stop; trip-level information applies.<br>`1` - Bikes are permitted to board/alight at this stop.<br>`2` - Bikes are not allowed to board/alight at this stop.<br>`3` - Bikes are permitted to board, but not alight, at this stop.<br>`4` - Bikes are permitted to alight, but not board, at this stop. |
